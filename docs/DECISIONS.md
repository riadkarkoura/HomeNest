...
**Addendum — Patch 8.2.1 (2026-07-16): closing the NULL-variant race.** A pre-payments architecture audit found that `UNIQUE (cart_id, product_id, variant_id)` never fires when `variant_id IS NULL` — standard SQL uniqueness semantics never treat two `NULL`s as equal — so it silently permitted duplicate `(cart_id, product_id, NULL)` rows under two concurrent "add to cart" requests for the same non-variant product (100% of the catalogue today has no variants). `incrementCartItem()` (`src/app/cart/actions.ts`) already assumed a unique violation would fire and fall back to an `UPDATE`; that fallback simply never engaged for this case. Fixed with a second, partial unique index — `cart_items_cart_product_no_variant_key`, `UNIQUE (cart_id, product_id) WHERE variant_id IS NULL` (migration `20260716000002_cart_items_null_variant_unique.sql`) — additive only, the original constraint is untouched and keeps protecting the has-variant case. No application code changed. A live check before applying confirmed zero pre-existing duplicate rows, so no data cleanup was needed; verified post-deploy by confirming a duplicate insert attempt is now rejected with a unique-violation error.
...
1. **`orders`/`order_items` gain their first-ever write policies.** Both tables existed since the initial schema (`20260711000002_schema.sql`) with SELECT-only RLS — nothing could ever insert an order. `orders_own_insert` (`WITH CHECK (auth.uid() = user_id)`) and `order_items_own_insert` (via the same `EXISTS (SELECT 1 FROM orders …)` correlation already used by that table's SELECT policy) enforce the same `auth.uid()`-owned-row authorization used everywhere else in this schema. No service-role key, consistent with ADR-013. **Naming note:** these two policy names put "own" before the action, diverging from this schema's otherwise exceptionless `{table}_{action}_own` convention for single-action policies — including the sibling `orders_select_own`/`order_items_select_own` on these same two tables. Confirmed cosmetic only (no runtime/RLS/query impact — policy names are never referenced by application code); a rename to `orders_insert_own`/`order_items_insert_own` is deferred to a future maintenance sprint rather than spending a migration on it now.
...
**Addendum — Patch 8.3.2, PaymentIntent concurrency guard (2026-07-18):** live Sprint 8.4 verification found that two near-simultaneous requests for the same order (client logs showed `/api/payments/stripe/intent` firing twice back-to-back per checkout) could both observe `orders.stripe_payment_intent_id = NULL`, both create a real Stripe PaymentIntent, and then race to write the result — whichever write landed last silently overwrote the other, permanently orphaning one PaymentIntent in Stripe with no order ever referencing it. The PaymentIntent-reuse fix above (item 4) only prevents duplicates *across separate requests over time* (a reload before paying); it does nothing for requests that are genuinely concurrent, since both read the same `NULL` before either has written back.

**Fix (optimistic concurrency, not locking):** `record_stripe_payment_intent()` (migration `20260718000001_stripe_intent_race_guard.sql`) now writes conditionally — `UPDATE orders SET stripe_payment_intent_id = ... WHERE ... AND stripe_payment_intent_id IS NULL` — and always returns the order's *current* value afterward, win or lose. Postgres implicitly row-locks whatever an `UPDATE` is about to touch, so two concurrent calls for the same order naturally serialize against that single write with no explicit `FOR UPDATE`, no held-open transaction, and no lock of any kind spanning the Stripe network call. The Route Handler compares the returned id against its own just-created one; if they differ, it retrieves and returns the *winner's* `client_secret` instead. The loser's own PaymentIntent is intentionally left alone — see the code comment in `route.ts` and the "PaymentIntent orphan cleanup" backlog item (`docs/ROADMAP.md`), which is explicitly operational-only and must never become part of this correctness path.

**Why not the other options considered:** row-locking across the Stripe call would need either a raw, manually-held Postgres transaction spanning two separate Route Handler `await`s (not supported by Supabase's stateless RPC client) or calling Stripe from inside a plpgsql function via an extension like `pg_net` — both new architecture for a narrow race. A bare `UNIQUE (stripe_payment_intent_id)` constraint doesn't address this failure mode at all (it stops the same id being linked to two different orders, not two different ids racing for the same order). Stripe idempotency keys were considered as the most direct upstream fix, but a key derived from `order.id` alone would risk Stripe replaying a stale ~24h-old cached response if a customer legitimately retries after a cancellation, quietly undermining the route's existing cancel/retry fallback (lines 51–53) — the DB-side fix achieves full correctness without that risk and stays entirely provider-agnostic.

---

## ADR-025 — AI Foundation adopted as HomeNest's AI architecture, preceding AI Search
**Date:** 2026-07-22
**Status:** Accepted

**Decision:** Four commits landed on `main` between Sprint 9.3 and this record — `feat(ai): create AI Core Foundation` (`9008348`), `feat(ai): implement provider abstraction layer` (`abaac0f`), and `feat(ai): implement context engine foundation` (`bf15665`), plus `feat(ai): implement AI orchestration layer` (`20084d2`) on a separate, not-yet-merged branch, `feat/ai-orchestration-layer` — are retroactively documented and formally adopted as **Sprint 10 — AI Foundation**, HomeNest's first concrete architectural layer toward the long-term direction ADR-017 already named. This ADR is a documentation reconciliation: the code was written and is accepted as-is; nothing here changes it.

**What was built, precisely:** four provider-agnostic layers under `src/ai/`, all contracts (TypeScript interfaces/types) plus a small number of concrete coordinating classes — no concrete AI vendor integration, no live API calls, no UI wiring:
1. **AI Core Foundation** (`src/ai/shared`, `context`, `prompts`, `memory`, `guardrails`, `telemetry`, `workflows`) — the shared vocabulary (`AIMessage`, `AIResult`, `AIError`, `AITokenUsage`) and one contract per future concern (a context fragment, a prompt template, a memory record, a guardrail policy, a telemetry event, a workflow step). Every file here is a `types.ts` re-exported through an `index.ts` barrel — no runtime logic.
2. **Provider Abstraction** (`src/ai/providers`) — the `AIProvider` interface every concrete vendor adapter (OpenAI, Anthropic, Google, Ollama — see the file-level comment in `types.ts`) will implement, plus the `AIProviderFactory` and `AIProviderRegistry` *contracts* (construction and discovery, respectively), `capabilities.ts` (a provider's declared feature set, e.g. `"streaming"`), and `resolution.ts`/`metadata.ts`/`errors.ts`. No concrete adapter class exists yet for any vendor.
3. **Context Engine** (`src/ai/context-engine`) — the one subsystem with a real, working coordinator: `DefaultContextEngine` (`engine.ts`) drives an injected resolver (which context categories a request needs), registry (which sources provide each category), and assembler (builds the final immutable `AIAssembledContext`), with an optional validator. No concrete source, resolver, or validator is registered yet — the engine has nothing to gather from until a feature (e.g. AI Search) supplies them.
4. **AI Orchestrator** (`src/ai/orchestration`, branch `feat/ai-orchestration-layer` only) — the second real coordinator: `PipelineOrchestrator` runs a request through a `SequentialPipeline` of `AIStage`s, threading context between stages, emitting lifecycle events, and supporting cooperative cancellation. Also contracts-plus-coordinator only — no stage exists yet for any concern.

**Reason — why this precedes AI Search rather than following it:** `docs/ROADMAP.md`'s original Sprint 9 ("AI Search") sketch had no abstraction layer in its task list — it assumed a Route Handler calling the Anthropic SDK directly (matching ADR-011's original, single-vendor plan). Building the provider-agnostic contract first means AI Search becomes the *first consumer* of `AIProvider`/`AIContextEngine` rather than the place those abstractions get extracted from after the fact — the same "build the seam before the second thing that needs it" reasoning already used for `src/lib/payments/` (ADR-022: Stripe was built behind a provider-agnostic boundary from the start specifically so PayPal/Klarna could be added later without touching checkout code). Sequencing the Foundation first means AI Search, AI Studio's `AIAssistantPanel` (Sprint 5.1, still disabled), and any future agent from `PROJECT_VISION.md`'s long list (Pricing, SEO, Marketing, etc.) all consume the same contracts instead of each hand-rolling its own Claude client — closing exactly the gap ADR-017 flagged ("favoring a clean server-side entry point a future agent could call... not a human-only UI flow").

**Reason — why this is architectural evolution, not scope drift:** scope drift would mean unplanned work bleeding into or destabilizing existing, shipped functionality. This does neither, verifiably:
- **Zero coupling.** `grep -rn "@/ai" src/app src/components src/lib` returns nothing — no existing route, component, Server Action, or query imports anything from `src/ai/` yet, and `src/ai/` imports nothing from the rest of the app. The storefront, checkout, Order Engine, and payments are provably untouched by these commits (confirmed by each commit's diffstat: every changed file is a new file under `src/ai/`).
- **Zero new runtime dependencies.** `package.json` gained no new packages — no `@anthropic-ai/sdk`, no HTTP client. This is type-level scaffolding, not a live integration; there is nothing here that could call a real model, incur cost, or fail at runtime.
- **Directly instructed.** Per explicit user instruction, 2026-07-22: "The apparent scope drift is intentional... has been intentionally adopted as the new architectural foundation of HomeNest AI. This is now an official architectural direction." This ADR is that instruction's formal record.
- **Consistent with, not contrary to, `PROJECT_VISION.md`.** That document's "Nothing here authorizes building AI now... not a task list" line is a guardrail against treating the long-term vision itself as standing authorization — it is not a prohibition on ever starting, and does not override an explicit, dated user decision to begin. The vision document is left unmodified (see Consequence) but now carries a pointer to this ADR so a future reader isn't misled into thinking the Foundation contradicts it.
- **Matches this project's own established pattern for foundational, contracts-only work landing ahead of its first consumer** — e.g. the `Review`/`Wishlist`/`Recommendation`/`Currency` type stubs in `src/types/index.ts` existed for multiple sprints before any sprint implemented them, by explicit design (`PROJECT_VISION.md`'s "Future Features" table).

**Relationship to ADR-011:** ADR-011 committed to a single vendor (Anthropic Claude, Haiku for search / Sonnet for content) with no abstraction layer contemplated. That decision is not reversed — Anthropic remains the intended first (and for now, only planned) concrete implementation — but it is now expressed as one adapter satisfying `AIProvider` rather than a hardcoded SDK call, so ADR-011's model assignments (`claude-haiku-4-5-20251001` / `claude-sonnet-4-6`) carry forward unchanged as the default `AIProviderConfig` a future `AnthropicProvider` adapter will use.

**Renumbering:** the original `docs/ROADMAP.md` "Sprint 9 — AI Search" collided with the already-shipped, unrelated Sprint 9.0–9.3 (UX Audit → Product Integrity → Brand Identity → Landing Page Transformation, all completed 2026-07-19–21) — the same kind of sprint-number reuse ADR-020 already precedented and resolved by disambiguating on date. AI Search is renumbered forward to **Sprint 11 — AI Runtime & Search Integration**, sequenced after this ADR's **Sprint 10 — AI Foundation**. See `docs/ROADMAP.md` §2/§3 for the updated sprint entries.

**Consequence:**
- `docs/ROADMAP.md`, `CLAUDE_CONTEXT.md`, `SESSION.md`, and `docs/ARCHITECTURE.md` (new §18) are updated alongside this ADR to reflect Sprint 10 as complete and to re-sequence the AI Search work as Sprint 11, now scoped as consuming the Foundation rather than building a Claude client from scratch.
- **The Orchestrator is accepted as complete work but is not yet on `main`** — it exists only on `feat/ai-orchestration-layer` (one commit, `20084d2`, plus a merge-from-main commit, currently fast-forwardable). Merging that branch is a code change and therefore explicitly outside the scope of this documentation-only reconciliation; it remains the one open action item this ADR does not close (tracked in `docs/ROADMAP.md` §3 and restated in this document's own status below).
- No existing ADR is superseded. No RLS policy, migration, Server Action, or component changed. The "No Service Role Key" (ADR-013), RLS-first, and provider-agnostic-boundary (ADR-022) precedents are all extended to the AI layer by this same decision, not exceptions to it.
- Upcoming work (Sprint 11) is now explicitly sequenced as: **Provider Runtime** (a concrete `AnthropicProvider` implementing `AIProvider`, per ADR-011's model assignments) → **Prompt Engine** (a concrete implementation of the `prompts` contract — template assembly, the system/user prompt structure `docs/ARCHITECTURE.md` §9.1 already sketched) → **Memory** and **Guardrails** (concrete implementations of their respective contracts — conversation/session memory, and output/safety policy enforcement) → **AI Search Integration** (the `/api/search` Route Handler, Upstash Redis cache, `search_logs` writes, and wiring the homepage's `AIConsultantSection` from keyword-matching to a real model call, per the original Sprint 9 task list in `docs/ROADMAP.md`).

**Alternatives considered:**
- **Treat the AI commits as an error and revert them** — rejected per explicit user instruction; the work is confirmed intentional, not accidental scope creep.
- **Leave the documentation gap as-is and let a future session rediscover it** — rejected; this is exactly the failure mode `CLAUDE_CONTEXT.md`'s "read this first" convention exists to prevent, and the gap was already surfaced in this session's prior architecture review.
- **Merge `feat/ai-orchestration-layer` into `main` as part of this reconciliation** — rejected for this task specifically: merging is a code/branch action, and this pass is scoped to documentation only, per explicit instruction ("Do NOT modify application code"). Recorded as the one remaining action item instead.
- **Fold the Orchestrator into "Upcoming" rather than "Completed"** since it isn't on `main` yet — rejected in favor of matching the user's own explicit classification (Completed: AI Core, Provider Abstraction, Orchestrator, Context Engine); its branch/merge status is called out precisely instead, so "completed" describes the work, not its merge state.
